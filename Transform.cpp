...
Vector2 Transform::GetWorldPosition()
{
	...
	{
		Vector3 objPosition_V3 = Vector3(position.x , position.y , 1);
		auto t = CameraManager::GetInstance()->GetRenderCamera()->GetTransform()->GetLocalToWorldMatrix().GetInverseMatrix();
		objPosition_V3 = Matrix3x3::Mul(objPosition_V3, CameraManager::GetInstance()->GetRenderCamera()->GetTransform()->GetLocalToWorldMatrix().GetInverseMatrix());
		position = Vector2(objPosition_V3.x , objPosition_V3.y );
	}
	while (true)
	{
		if (parent == nullptr)
